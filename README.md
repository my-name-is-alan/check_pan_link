...
当前项目的重点已经放在 115 和 123 分享链接上，既支持基础可用性检查，也支持直接拉取分享文件列表，并提供了一个内置 demo 页面方便手工测试。
The current focus of this project is 115 and 123 share links. It supports both basic availability checks and file list retrieval, and includes a built-in demo page for manual testing.

## Current Capabilities / 当前能力

- `POST /api/check`
  用来检查分享链接是否可用，返回 `valid / invalid / processing / unknown` 四态结果。
  Checks whether a share link is usable and returns one of `valid / invalid / processing / unknown`.
- `POST /api/115/share/list`
  用来获取 115 分享链接的文件列表。  
  Fetches the file list for a 115 share link.
- `POST /api/123/share/list`
  用来获取 123 云盘分享链接的文件列表。
  Fetches the file list for a 123 Pan share link.
- `GET /demo`
  内置一个简单的测试页面，可以直接在浏览器里测试 `/api/check`、`/api/115/share/list` 和 `/api/123/share/list`。
  Serves a built-in demo page for testing `/api/check`, `/api/115/share/list`, and `/api/123/share/list` in the browser.
...
- [x] 内置 demo 页面
  方便直接拿项目里的示例 115 / 123 链接做测试。
  Includes a built-in demo page with sample 115 / 123 links for quick testing.
- [x] 115 实际返回兼容  
...
- [x] 123 分享链接状态检查
  支持 `123865.com` 等旧分享域名，并通过 123 的分享信息接口判断链接有效、失效、提取码错误等状态。
  123 share status checks with legacy share domains such as `123865.com`, using 123's share info APIs instead of plain HTTP status.
- [x] 123 分享文件列表接口
  支持从分享链接递归拉取文件内容，并返回 `files` 或 `tree` 结构。
  123 share file listing with recursive traversal and `files` / `tree` output.
...
## 123 Examples / 123 使用示例

### Check a 123 Share / 检查 123 分享链接

```bash
curl -X POST http://127.0.0.1:8080/api/check \
  -H "content-type: application/json" \
  -d "{\"url\":\"https://www.123865.com/s/IpPUVv-gGKj?pwd=Ocat\"}"
```

### List 123 Files / 获取 123 文件列表

```bash
curl -X POST http://127.0.0.1:8080/api/123/share/list \
  -H "content-type: application/json" \
  -d "{\"url\":\"https://www.123865.com/s/IpPUVv-gGKj?pwd=Ocat\",\"list_type\":\"files\"}"
```

### Get 123 Tree Structure / 获取 123 树形目录

```bash
curl -X POST http://127.0.0.1:8080/api/123/share/list \
  -H "content-type: application/json" \
  -d "{\"url\":\"https://www.123865.com/s/IpPUVv-gGKj?pwd=Ocat\",\"list_type\":\"tree\"}"
```
...
- `providers/`
  各网盘 provider 逻辑，当前重点是 115 / 123。
  Provider implementations, currently focused on 115 / 123.
- `routes/`
...
- [ ] 为 `pan189` 增加像 115 / 123 一样的 provider 级状态识别，而不是只做基础 HTTP 检查。
  Add provider-specific status detection for `pan189`, instead of relying only on basic HTTP checks.
- [ ] 增加批量检查接口，支持一次提交多条链接。  
...
这个项目目前已经可以拿来做 115 / 123 分享链接的日常检查和文件树测试，但整体还在持续演进阶段。
This project is already usable for day-to-day 115 / 123 share checking and file tree inspection, but it is still evolving.

如果你现在主要关注 115 / 123：
If your main focus right now is 115 / 123:

- `/api/check` 适合做链接状态判断。  
  `/api/check` is best for link status checks.
- `/api/115/share/list` 和 `/api/123/share/list` 适合做目录抓取。
  `/api/115/share/list` and `/api/123/share/list` are best for file and folder listing.
- `/demo` 适合做手工联调和快速验证。  
